...
2. Deploy server and privacy/terms changes with the mobile build. Accounts must
   accept policy version `2026-09-21.1` before sharing. Terms are a short, skippable
   onboarding screen and remain available in Profile. If skipped, Send opens a
   compact acceptance modal before uploading or leaving the draft. Acceptance
   resumes that send; dismissal keeps the draft and recipients. Account deletion,
   blocking, and reporting remain available without acceptance.

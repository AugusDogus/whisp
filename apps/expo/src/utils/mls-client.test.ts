...
let nativeEnqueue: (input: string) => Promise<string> = async () => messageId;
let nativeConfigure: () => Promise<void> = async () => {};
...
mock.module("react-native-whisp-mls", () => ({
  ...
  nativeSend: {
    ...
    enqueue: (input: string) => nativeEnqueue(input),
  },
  ReceivedMessage: {},
  initializeMls: () => {},
...
test("warm native enqueue reuses configuration without a session lookup", async () => {
  ...
  expect(nativeConfigurations).toEqual([]);
});
...
test("native enqueue reconfigures after an account switch", async () => {
  ...
  expect(sessionReads).toBe(1);
  expect(nativeConfigurations).toHaveLength(1);
  expect(JSON.parse(nativeConfigurations[0] ?? "null")).toMatchObject({
    userId: "bob",
  });
});
